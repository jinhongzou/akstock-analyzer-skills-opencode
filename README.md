...
- 🀆 **19 个独立 Skill**：可按需单独使用，也可一键输出完整投资报告
...
| **入口层** | `core/src/skills/` | 16 个 skill 的 `main.py`，参数解析 + 格式化输出 |
...
- 🀆 **19 Independent Skills**: Use individually or combine for a full investment report
...
│       │       ├── analyzers/           # Analyzer layer, 8 Analyzer classes
│       │       │   ├── __init__.py
...
│       │       └── skills/              # 16 skill entry points (thin wrapper)
│       │           ├── stock-analyzer/main.py
│       │           ├── technical-analyzer/main.py
│       │           ├── a-dividend-analyzer/main.py
│       │           ├── buffett-checklist/main.py
...
|-------|-----------|----------------|
| **Backward Compat** | `core/__init__.py` | 26 wrapper functions + 9 class exports, delegates to Analyzer classes |
| **Analyzers** | `core/src/analyzers/` | 8 Analyzer classes, data fetching + business logic |
| **Infrastructure** | `core/src/infra/` | CacheManager + ReportGenerator |
| **Entry Points** | `core/src/skills/` | 16 skill `main.py` files, argument parsing + formatted output |
